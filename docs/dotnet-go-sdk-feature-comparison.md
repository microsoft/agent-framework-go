...
| Compaction and chat reduction | Compaction provider, triggers, message index/groups, sliding window, context window, truncation, summarization, tool-result, pipeline, chat reducer adapter. | `agent/compaction` provider, triggers, message index/groups, sliding window, truncation, summarization, tool-result, pipeline. | Partial | Go lacks the .NET `ContextWindowCompactionStrategy` and `IChatReducer` adapter surface; otherwise the major strategies line up. |
...
| AGUI hosting | ASP.NET Core AGUI hosting, end-to-end web chat samples. | `agent/hosting/aguihosting`, JSON HTTP handler, backend/frontend tools, HITL, state examples. | Partial | Go has handlers and examples, but no ASP.NET/Blazor-style end-to-end web app equivalent. |
...
| Agent workflow builders | Sequential, concurrent, handoff, group chat builders. | Manual builder plus `AddChain`, `AddSwitch`, direct/fan-out/fan-in edges; workflow-as-agent and agents-in-workflows examples. | Partial | Go lacks first-class handoff and group chat builders; sequential/concurrent can be composed manually. |
| Workflow graph builder | `WorkflowBuilder`, direct edges, fan-out, fan-in barrier, labels, conditions, switch/case samples. | `workflow.Builder`, `AddEdge`, `AddDirectEdge`, `AddFanOutEdge`, `AddFanInBarrierEdge`, `WithLabel`, `WithAssigner`, `AddSwitch`. | Aligned | .NET has more overloads/extension methods; Go uses simpler methods and option functions. |
| Workflow executor model | Generic `Executor<TInput>` and `Executor<TInput,TOutput>`, function executors, aggregating executor, protocol builder. | `Executor`, `ExecutorConfig`, `ExecutorOptions`, `RouteBuilder`, `BindFunc`, `BindFuncContext`, `StatefulExecutorCache`. | Partial | .NET has more overloads and an explicit `AggregatingExecutor`; Go has a stateful cache and route builder but fewer convenience bind overloads. |
| Workflow protocol description | Accepts/yields/sends/catch-all protocol descriptor and chat protocol helpers. | `ProtocolDescriptor` currently exposes accepted input types; route builder tracks output types internally. | Partial | Go protocol reflection is narrower. It does not expose the full .NET sent/yielded/catch-all descriptor shape. |
...
| Samples and tutorials | Very broad sample set: get started, providers, agents, skills, Foundry, memory, RAG, AGUI, A2A, MCP, declarative, DevUI, evaluation, durable hosting, web chat, Purview, M365. | Focused sample set: get started, providers, agents, A2A, AGUI, MCP server, skills, workflows, A2A end-to-end, chat CLI. | Partial | Go samples cover core parity areas but miss many .NET integration scenarios. |
