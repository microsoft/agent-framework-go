...
| Foundry agents and hosted agents | `Microsoft.Agents.AI.Foundry`, `Foundry.Hosting`, Foundry agent lifecycle and hosted agent samples. | `provider/foundryprovider` for Foundry memory only. | .NET only | Go lacks Foundry hosted agent lifecycle, invocation, and server-side tool hosting integrations. |
...
| `provider/foundryprovider` | Microsoft Foundry memory provider. |
