...
- Both Go and .NET support the three skill tools (`load_skill`, `read_skill_resource`, `run_skill_script`); in Go, they are registered when skills are discovered or provided, and when present the tools return descriptive errors if the requested resource or script is not found. Go also exposes the tool-name constants (`skills.LoadSkillToolName`, `skills.ReadSkillResourceToolName`, `skills.RunSkillScriptToolName`), reusable auto-approval helpers (`skills.ReadOnlyToolsAutoApprovalRule`, `skills.AllToolsAutoApprovalRule`), per-tool approval controls (`DisableLoadSkillApproval`, `DisableReadSkillResourceApproval`, `DisableRunSkillScriptApproval`), and `ContextProviderOptions.IncludeDetailedErrors`, matching the corresponding .NET `AgentSkillsProviderOptions` surface.
