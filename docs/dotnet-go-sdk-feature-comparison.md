...
| A2A hosting | A2A hosting packages, ASP.NET Core hosting, samples. | `provider/a2aprovider`, executor for a2a-go JSON-RPC and JSON HTTP handlers, isolation-key scoped task-store wrapper for multi-tenant hosts, end-to-end client/server sample. | Partial | Go now includes task-store isolation scoping for multi-tenant A2A hosts, but it does not provide ASP.NET-style hosting/DI integration. |
