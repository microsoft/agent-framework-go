...
| Agent identity and metadata | `AIAgentMetadata`, agent ID/name/description, source attribution extensions. | Agent ID/name/description, provider name, response author stamping, `message.Message.Source`, and `(*message.Message).WithSource`. | Aligned | API shape differs: .NET exposes extension methods around `Microsoft.Extensions.AI`; Go exposes source data through an explicit `message.Source` field and a `WithSource` helper. |
...
| Chat history | `ChatHistoryProvider`, `InMemoryChatHistoryProvider`, per-service-call persistence, reducer triggers. | `HistoryProvider`, default in-memory history for local sessions, compaction-backed history provider, third-party storage example. | Partial | Go now has first-class reducer-trigger support through `agent/compaction.NewHistoryProvider`, but it still has fewer built-in storage providers than .NET. |
...
| Memory integrations | Chat history memory, bounded chat history, Mem0, Foundry memory, RAG samples, file memory. | In-memory history/context examples and `foundryprovider.NewMemoryProvider`. | Partial | Go has Foundry memory and primitives to build memory, but no Mem0, RAG, bounded memory package, or file memory provider. |
...
| Hosted/server-side tools | Foundry/OpenAI samples for code interpreter, file search, web search, OpenAPI, Bing custom search, SharePoint, Microsoft Fabric, memory search, Toolbox, hosted MCP. | `tool/hostedtool` declarations for web search, file search, code interpreter, MCP server; Foundry-first samples cover code interpreter, web search, MCP client tools, and local MCP tools; the OpenAI provider sample covers code interpreter with container-file download. | Partial | Go has declaration types and initial Foundry/OpenAI Responses hosted-tool coverage, but fewer service-specific Foundry hosted tool integrations and no Foundry toolbox lifecycle sample. |
...
| MCP client tools | MCP client tools over local stdio and remote HTTP transports, with agent samples. | `mcptool.Connect`, `ListTools`, wrappers over MCP client sessions, and remote HTTP and local stdio agent samples. | Aligned | Both SDKs expose remote MCP tools to agents; .NET's additional declarative MCP integration is tracked under declarative workflows below. |
...
| OpenAI provider | OpenAI Chat Completions, Responses, Azure OpenAI, background responses, code interpreter file download samples. | `openaiprovider.NewAgent`, `NewChatCompletionsAgent`, `NewResponsesAgent`, Azure OpenAI through the OpenAI Go Azure client, continuation/background response support, and OpenAI samples matching all six numbered and two unnumbered .NET provider scenarios. | Aligned | API shape differs: Go passes the root `openai.Client` to provider constructors instead of wrapping API-specific .NET client types. |
...
| Workflow streaming and runs | `Run`, `StreamingRun`, open/run/resume streaming, try-send, run to halt, status, events. | `inproc.Run`, `StreamingRun`, `Run`, `OpenStreaming`, `RunStreaming`, `Resume`, `ResumeStreaming`, `TrySendMessage`, `WatchStream`, `WatchUntilHalt`, status. | Aligned | Naming follows Go conventions (`SessionID` instead of .NET `SessionId`). Incompatible try-send input returns false without an error. |
| Workflow checkpointing | In-memory and JSON checkpoint managers, custom stores, Cosmos store, checkpoint restore, checkpoint hooks. | In-memory checkpoint manager (`checkpoint.NewInMemoryManager`), JSON+file checkpoint manager (`checkpoint.NewJSONManager` with `checkpoint.FileSystemJSONStore`), `Manager.LatestCheckpoint`, custom store interface (`checkpoint.Store[json.RawMessage]`), `WithCheckpointing`, checkpoint restore, checkpoint hooks, resume pending request republish, checkpoint-and-rehydrate example. | Partial | Go lacks a Cosmos store. Custom durable stores can be implemented via the public `checkpoint.Store[json.RawMessage]` interface. |
...
| Agent in workflow | `AIAgentBinding`, `AIAgentHostOptions`, response/update events, role reassignment, message forwarding, intercept user-input/function-call requests. | `workflow/agentworkflow.New` with `Config`: `EmitUpdateEvents`, `EmitResponseEvents`, pointer-backed `ForwardIncomingMessages` and `ReassignOtherAgentsAsUsers`, `InterceptUserInputRequests`, `InterceptUnterminatedFunctionCalls`. | Aligned | Go uses pointers for nullable/default-true settings so omission preserves .NET defaults while explicit `new(false)` remains available. |
| Workflow as agent | Workflow host agent / `AsAIAgent`, experimental session checkpoint recovery service, sample. | `agentworkflow.NewAgent` with session-backed checkpoint rehydration. | Partial | Go chooses the in-process environment based on concurrency and intentionally omits .NET's experimental `WorkflowSessionCheckpointRecovery` API. |
...
- Go and .NET both support background-response polling and streaming resumption through continuation tokens. Go keeps its runnable background-response example with the Azure OpenAI provider, while .NET places its examples in the general agent sample set.
...
- Go hosted tools should be treated as provider-dependent declarations. OpenAI demonstrates code interpreter with container-file download, while Foundry demonstrates code interpreter and web search. The .NET samples add more service-specific Foundry scenarios.
...
- Both Go and .NET support the three skill tools (`load_skill`, `read_skill_resource`, `run_skill_script`); in Go, they are registered when skills are discovered or provided, and when present the tools return descriptive errors if the requested resource or script is not found. Go also exposes reusable auto-approval helpers (`skills.ReadOnlyToolsAutoApprovalRule`, `skills.AllToolsAutoApprovalRule`), per-tool approval controls (`DisableLoadSkillApproval`, `DisableReadSkillResourceApproval`, `DisableRunSkillScriptApproval`), and `ContextProviderOptions.IncludeDetailedErrors`, matching the corresponding .NET `AgentSkillsProviderOptions` surface.
...
| `agent/harness/loop` | Loop reinvocation harness with delegate and completion-marker evaluators. |
...
| `provider/otelprovider` | Agent OpenTelemetry middleware. |
| `provider/openaiprovider` | OpenAI Chat Completions and Responses, including Azure OpenAI client usage. |
| `provider/anthropicprovider` | Anthropic provider. |
| `provider/geminiprovider` | Gemini provider. |
| `provider/foundryprovider` | Microsoft Foundry project Responses agents, server-side agent endpoint invocation, client headers, hosted-agent user-identity pass-through, served-model metadata, and memory provider. |
| `provider/a2aprovider` | A2A remote agent provider and hosting executor for a2a-go handlers. |
| `provider/aguiprovider` | AGUI remote agent provider and hosting handler/events. |
| `workflow/agentworkflow` | Workflow-as-agent and agent-as-workflow-executor adapters; sequential, concurrent, and group chat agent workflow builders. |
