...
| Shell tool | `Microsoft.Agents.AI.Tools.Shell`: `LocalShellTool`, `ShellPolicy` (deny-list), `ShellResult`, stateless and persistent shell execution modes, approval-in-the-loop gate, head-tail output truncation. | `tool/shelltool.New`, `shelltool.Options` (mode, timeout, max output, policy, acknowledge unsafe), `shelltool.Policy`, `shelltool.Result.FormatForModel`. | Aligned | Go implementation mirrors the .NET design: policy deny-list, approval-required by default, stateless and persistent modes, head-tail truncation. Docker shell executor not ported (Go has no equivalent `DockerShellExecutor`). |
...
| `tool/shelltool` | Local shell command execution tool with policy deny-list, approval gate, and output truncation. |
