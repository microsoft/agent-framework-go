...
| Memory integrations | Chat history memory, bounded chat history, Mem0, Foundry memory, RAG samples, file memory. | In-memory history/context examples, `foundryprovider.NewMemoryProvider`, `agent/harness/filestore`, and `agent/harness/filememory`. | Partial | Go now has Foundry memory plus session-scoped file memory and file-store primitives, but still lacks Mem0, RAG, and a bounded-memory package. |
...
| Harness utilities | Agent mode, file access, file memory, file store, subagents, todo, tool approval harness providers, loop harness. | `agent/harness/agentmode`, `agent/harness/todo`, `agent/harness/toolapproval`, `agent/harness/toolautocall`, `agent/harness/loop`, `agent/harness/filestore`, `agent/harness/filememory`. | Partial | Go now has packaged harness support for agent mode, todo tracking, tool approval, tool auto-call, loop reinvocation with delegate/completion-marker evaluators, plus file-store primitives and session-scoped file memory tools. It still lacks file access, subagent harness utilities, and the .NET AI-judge loop evaluator. Agent mode tool names (`mode_set`/`mode_get`), default instructions, and mode descriptions are aligned with .NET (#6071). |
