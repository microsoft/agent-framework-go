...
| Workflow graph builder | `WorkflowBuilder`, direct edges, fan-out, fan-in barrier, labels, conditions, switch/case samples; `WithOutputFrom`, `WithIntermediateOutputFrom`, `OutputTag`, `OutputTag.Intermediate`. | `workflow.Builder`, `AddEdge`, `AddDirectEdge`, `AddFanOutEdge`, `AddFanInBarrierEdge`, `WithEdgeLabel`, `WithEdgeAssigner`, `AddSwitch`; `WithOutputFrom`, `WithIntermediateOutputFrom`, `OutputTagIntermediate`, `OutputEvent.Tags`, `OutputEvent.IsIntermediate`. | Aligned | .NET has more overloads/extension methods; Go uses simpler methods and option functions. Output tagging now aligned. |
