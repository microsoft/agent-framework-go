...
func TestExternalResponse_RejectsForgedPortIDWithoutConsumingRequest(t *testing.T) {
	...
	if _, err := run.Resume(ctx, forged); err != nil {
		t.Fatalf("Resume with forged response: %v", err)
	}
	var sawPortMismatch bool
	for evt := range run.NewEvents() {
		if errEvt, ok := evt.(workflow.ErrorEvent); ok {
			if errEvt.Error != nil && strings.Contains(errEvt.Error.Error(), "response port id") {
				sawPortMismatch = true
			}
		}
	}
	if !sawPortMismatch {
		t.Fatal("expected ErrorEvent for port mismatch")
	}
	status, err := run.GetStatus(ctx)
	if err != nil {
		t.Fatalf("GetStatus after forged response: %v", err)
	}
	if status == inproc.RunStatusEnded {
		t.Fatal("run ended after rejected forged response")
	}
