...
func TestConcurrentWorkflowBuilder_ReturnsErrorForNoAgents(t *testing.T) {
	_, err := agentworkflow.NewConcurrentWorkflowBuilder().Build()
	if err == nil {
...
func TestConcurrentWorkflowBuilder_ReturnsErrorForNilAgent(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			_, err := agentworkflow.NewConcurrentWorkflowBuilder(tt.agents...).Build()
			if err == nil {
...
func TestConcurrentWorkflowBuilder_SingleAgent(t *testing.T) {
	...
	wf, err := agentworkflow.NewConcurrentWorkflowBuilder(a).WithName("single-concurrent").Build()
	if err != nil {
...
func TestConcurrentWorkflowBuilder_MultiAgent(t *testing.T) {
	...
	wf, err := agentworkflow.NewConcurrentWorkflowBuilder(a, b).Build()
	if err != nil {
		t.Fatalf("Build: %v", err)
	}
...
func TestConcurrentWorkflowBuilder_ExplicitOutputDesignationSuppressesDefaults(t *testing.T) {
	a := newLabeledEchoAgent("a", "A", "from-a")
	b := newLabeledEchoAgent("b", "B", "from-b")
	wf, err := agentworkflow.NewConcurrentWorkflowBuilder(a, b).
		WithOutputFrom(a).
		Build()
	if err != nil {
		t.Fatalf("Build: %v", err)
	...
	wantOutputID := agentworkflow.New(a, agentworkflow.Config{}).ID
...
func TestConcurrentWorkflowBuilder_ExplicitOutputDesignationRejectsNonParticipant(t *testing.T) {
	...
	for _, testCase := range []struct {
		name      string
		configure func(*agentworkflow.ConcurrentWorkflowBuilder) *agentworkflow.ConcurrentWorkflowBuilder
	}{
		{
			name: "terminal",
			configure: func(builder *agentworkflow.ConcurrentWorkflowBuilder) *agentworkflow.ConcurrentWorkflowBuilder {
		...
		t.Run(testCase.name, func(t *testing.T) {
			_, err := testCase.configure(agentworkflow.NewConcurrentWorkflowBuilder(a)).Build()
			if err == nil {
...
func TestConcurrentWorkflowBuilder_UsesCustomAggregator(t *testing.T) {
	...
	wf, err := agentworkflow.NewConcurrentWorkflowBuilder(a, b).WithAggregator(func(ctx context.Context, lists [][]*message.Message) []*message.Message {
...
func TestConcurrentWorkflowBuilder_ResetsEndExecutorBetweenTurns(t *testing.T) {
	...
	b := newLabeledEchoAgent("b", "B", "from-b")
	callCount := 0
	wf, err := agentworkflow.NewConcurrentWorkflowBuilder(a, b).WithAggregator(func(_ context.Context, lists [][]*message.Message) []*message.Message {
		callCount++
		return []*message.Message{{
			Role: message.RoleAssistant,
			Contents: []message.Content{
...
func TestConcurrentWorkflowBuilder_AgentsRunInParallel(t *testing.T) {
	barrier := &runBarrier{}
	wf, err := agentworkflow.NewConcurrentWorkflowBuilder(
