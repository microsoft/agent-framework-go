		...
		for _, c := range m.Contents {
			if id, ok := responseContentID(c); ok {
				...
				if request, found := pending[id]; found {
					if request == nil {
						return nil, nil, false, fmt.Errorf("agentworkflow: pending response %q has no external request", id)
					}
...
func normalizeResponseData(response message.Content, request *workflow.ExternalRequest) (any, error) {
	if request == nil {
		return nil, errors.New("agentworkflow: pending request has no external request")
	}
	...
	case *message.FunctionResultContent:
		...
		if !request.PortInfo.ResponseType.MatchPolymorphic(reflect.TypeFor[*message.FunctionResultContent]()) {
			if r.Result == nil {
				return nil, fmt.Errorf("agentworkflow: null result is not supported for request port %q with response type %v", request.PortInfo.PortID, request.PortInfo.ResponseType)
			}
			if request.PortInfo.ResponseType.MatchPolymorphic(reflect.TypeOf(r.Result)) || isPortableValueResult(r.Result) {
				return r.Result, nil
			}
			return nil, fmt.Errorf("agentworkflow: unexpected result type in FunctionResultContent %T; expecting %v", r.Result, request.PortInfo.ResponseType)
		}
	case *message.ToolApprovalResponseContent:
...
func requestToUpdate(req *workflow.ExternalRequest, responseID string, raw any) (*agent.ResponseUpdate, string, *workflow.ExternalRequest, bool, error) {
	...
	if !ok {
		...
		if err != nil {
			return nil, "", nil, false, fmt.Errorf("agentworkflow: failed to surface external request %q: %w", req.RequestID, err)
		}
