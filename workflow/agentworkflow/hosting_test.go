...
func TestHostedAgent_BindingIDUsesAgentNameWhenProvided(t *testing.T) {
	agentA := agentworkflow.New(newNamedNoopAgent("agent-a-id", "AgentA"), agentworkflow.Config{})
	agentB := agentworkflow.New(newNamedNoopAgent("agent-b-id", "AgentB"), agentworkflow.Config{})
...
func TestHostedAgent_BindingIDUsesAgentIDWhenNameMissing(t *testing.T) {
	agentA := agentworkflow.New(newNamedNoopAgent("agent-a-id", ""), agentworkflow.Config{})
	agentB := agentworkflow.New(newNamedNoopAgent("agent-b-id", ""), agentworkflow.Config{})
	wf, err := workflow.NewBuilder(agentA).AddEdge(agentA, agentB).Build()
	if err != nil {
		t.Fatalf("Build: %v", err)
	}
...
func TestHostedAgent_BindingIDSanitizesNameAndID(t *testing.T) {
	binding := agentworkflow.New(newNamedNoopAgent("agent id", "Agent A!"), agentworkflow.Config{})
...
func runHostedAgent(t *testing.T, a *agent.Agent, cfg agentworkflow.Config, token workflow.TurnToken, msgs []*message.Message) []workflow.Event {
	t.Helper()
	binding := agentworkflow.New(a, cfg)
	...
	for _, executorSetting := range []bool{true, false} {
		t.Run(fmt.Sprintf("emit=%v", executorSetting), func(t *testing.T) {
			events := runHostedAgent(t, newReplayAgent(),
				agentworkflow.Config{EmitResponseEvents: executorSetting},
				workflow.TurnToken{},
...
func TestHostedAgent_RoleReassignmentSkipsNonConversationalAssistantMessages(t *testing.T) {
	...
	runHostedAgent(t, newRecordingAgent(&calls), agentworkflow.Config{}, workflow.TurnToken{}, []*message.Message{textMsg, callMsg})
...
func TestHostedAgent_HandlesSingleMessage(t *testing.T) {
	...
	binding := agentworkflow.New(newRecordingAgent(&calls), agentworkflow.Config{})
	wf, err := workflow.NewBuilder(binding).Build()
	if err != nil {
		t.Fatalf("build workflow: %v", err)
	}
...
func TestHostedAgent_InterceptUnterminatedFunctionCalls(t *testing.T) {
	host := agentworkflow.New(newFunctionCallAgent(), agentworkflow.Config{
...
func TestHostedAgent_ResetSignal_StartsNewSession(t *testing.T) {
	...
	sendStreamMessage(t, stream, ctx, agentworkflow.ResetSignal{})
	for _, err := range stream.WatchUntilHalt(ctx) {
		if err != nil {
...
func TestHostedAgent_InterceptsOnlyUnpairedFunctionCalls_PortMode(t *testing.T) {
	...
	host := agentworkflow.New(newRequestAgent(unpaired, paired), agentworkflow.Config{
		EmitResponseEvents: true,
	})
	wf, err := workflow.NewBuilder(host).WithOutputFrom(host).Build()
	if err != nil {
