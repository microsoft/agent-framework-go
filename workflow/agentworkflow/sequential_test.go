...
func TestSequentialWorkflowBuilder_ReturnsErrorForNoAgents(t *testing.T) {
	_, err := agentworkflow.NewSequentialWorkflowBuilder().Build()
	if err == nil {
...
func TestSequentialWorkflowBuilder_ReturnsErrorForNilAgent(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			_, err := agentworkflow.NewSequentialWorkflowBuilder(tt.agents...).Build()
			if err == nil {
...
func TestSequentialWorkflowBuilder_SingleAgent(t *testing.T) {
	...
	wf, err := agentworkflow.NewSequentialWorkflowBuilder(a).WithName("single-agent").Build()
	if err != nil {
...
func TestSequentialWorkflowBuilder_MultiAgent(t *testing.T) {
	...
	wf, err := agentworkflow.NewSequentialWorkflowBuilder(a, b, c).Build()
	if err != nil {
		t.Fatalf("Build: %v", err)
	}
	if wf.Name() != "" {
...
func TestSequentialWorkflowBuilder_ExplicitOutputDesignationSuppressesDefaults(t *testing.T) {
	a := newLabeledEchoAgent("a", "A", "from-a")
	b := newLabeledEchoAgent("b", "B", "from-b")
	wf, err := agentworkflow.NewSequentialWorkflowBuilder(a, b).
		WithOutputFrom(b).
		Build()
	if err != nil {
		t.Fatalf("Build: %v", err)
	...
	wantOutputID := agentworkflow.New(b, agentworkflow.Config{}).ID
...
func TestSequentialWorkflowBuilder_ExplicitOutputDesignationRejectsNonParticipant(t *testing.T) {
	...
	for _, testCase := range []struct {
		name      string
		configure func(*agentworkflow.SequentialWorkflowBuilder) *agentworkflow.SequentialWorkflowBuilder
	}{
		{
			name: "terminal",
			configure: func(builder *agentworkflow.SequentialWorkflowBuilder) *agentworkflow.SequentialWorkflowBuilder {
		...
		t.Run(testCase.name, func(t *testing.T) {
			_, err := testCase.configure(agentworkflow.NewSequentialWorkflowBuilder(a)).Build()
			if err == nil {
...
func TestSequentialWorkflowBuilder_AgentsRunInOrder(t *testing.T) {
	for _, numAgents := range []int{1, 2, 3, 4, 5} {
		t.Run(fmt.Sprintf("%d_agents", numAgents), func(t *testing.T) {
			...
			wf, err := agentworkflow.NewSequentialWorkflowBuilder(agents...).Build()
			if err != nil {
				t.Fatalf("Build: %v", err)
			}
...
func TestSequentialWorkflowBuilder_ChainOnlyAgentResponses(t *testing.T) {
	...
	wf, err := agentworkflow.NewSequentialWorkflowBuilder(agents...).WithChainOnlyAgentResponses(true).Build()
	if err != nil {
		t.Fatalf("Build: %v", err)
	}
