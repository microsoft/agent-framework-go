...
func TestChain(t *testing.T) {
	t.Run("empty middleware chain", func(t *testing.T) {
		...
		seq := middleware.RunChain(ctx, fn, nil, []*message.Message{})
		for range seq {
		}
	...
	t.Run("single middleware", func(t *testing.T) {
		...
		ctx := context.Background()
		seq := middleware.RunChain(ctx, fn, []middleware.Middleware{mw}, []*message.Message{})
		for range seq {
		}
	...
	t.Run("multiple middlewares", func(t *testing.T) {
		...
		ctx := context.Background()
		seq := middleware.RunChain(ctx, fn, []middleware.Middleware{mw1, mw2, mw3}, []*message.Message{})
		for range seq {
		}
...
func (im *interceptMiddleware) Run(next middleware.RunFunc, ctx context.Context, messages []*message.Message, options ...agentopt.Option) iter.Seq2[*message.ResponseUpdate, error] {
	return func(yield func(*message.ResponseUpdate, error) bool) {
		count := 0
