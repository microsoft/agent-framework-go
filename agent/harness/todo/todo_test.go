...
// TestCompleteTodos_EmptyReasonIsRejected verifies that TodoList_Complete
// returns an error when a completion entry has an empty or whitespace-only reason.
func TestCompleteTodos_EmptyReasonIsRejected(t *testing.T) {
	cases := []struct {
		name   string
		reason string
	}{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			callTool(t, outOpts, "TodoList_Add", `{"Arg0":[{"title":"Task Y"}]}`)
			items := p.GetAllItems(opts...)
			if len(items) != 1 {
				t.Fatalf("expected 1 item, got %d", len(items))
			}

			argsJSON := fmt.Sprintf(`{"Arg0":[{"id":%d,"reason":%q}]}`, items[0].ID, tc.reason)

			var completeTool tool.FuncTool
			for _, opt := range outOpts {
				if tt, ok := opt.Value().(tool.Tool); ok && tt.Name() == "TodoList_Complete" {
					completeTool = tt.(tool.FuncTool)
					break
				}
			}
			if completeTool == nil {
				t.Fatal("TodoList_Complete tool not found")
			}

			_, err = completeTool.Call(tool.Context{Context: context.Background()}, argsJSON)
			if err == nil {
				t.Errorf("expected error for %q reason, got nil", tc.reason)
			}

			// The item must not have been marked complete.
			all := p.GetAllItems(opts...)
			if all[0].IsComplete {
				t.Error("item should not be marked complete when reason is invalid")
			}
		})
	}
}
