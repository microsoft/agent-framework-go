...
func TestFileSkill_WithScripts_ContentIncludesAvailableScriptsBlock(t *testing.T) {
	...
	if !strings.Contains(content, "<available_resources />") {
		t.Fatalf("expected empty <available_resources /> peer in content, got: %s", content)
	}
}
...
func TestFileSkill_WithoutResourcesOrScripts_ContentIncludesSelfClosingBlocks(t *testing.T) {
	root := t.TempDir()
...
func TestFileSkill_ScriptContent_IncludesDefaultArraySchema(t *testing.T) {
	...
	if !strings.Contains(content, "<parameters_schema>") {
		t.Fatalf("expected parameters schema element in content, got: %s", content)
	}
