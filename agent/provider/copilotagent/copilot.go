...
// Config contains configuration for a GitHub Copilot-backed [agent.Agent].
type Config struct {
	agent.Config
...
// New creates an agent backed by the GitHub Copilot SDK.
func New(cclient *copilot.Client, config Config) *agent.Agent {
	if cclient == nil {
		panic("copilotagent: client cannot be nil")
	}
	if config.Name == "" {
...
func (p *provider) run(ctx context.Context, messages []*message.Message, options ...agent.Option) iter.Seq2[*agent.ResponseUpdate, error] {
	return func(yield func(*agent.ResponseUpdate, error) bool) {
		if p.client == nil {
			yield(nil, errors.New("copilotagent: client cannot be nil"))
			return
		}
