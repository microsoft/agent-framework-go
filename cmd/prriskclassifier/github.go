...
var managedLabels = []riskLabel{
	...
	{Name: failedAutoRisk, Color: "6E7781", Description: "Automatic risk classification needs maintainer attention"},
}
...
func (c *ghClient) ensureLabels(ctx context.Context) error {
	out, err := c.run(ctx, c.withRepo("label", "list", "--limit", "1000", "--json", "name")...)
	if err != nil {
		return err
	}
	var existing []struct {
...
func (c *ghClient) pullRequestSignals(ctx context.Context, number int) ([]string, []string, error) {
	out, err := c.run(ctx, c.withRepo("pr", "view", strconv.Itoa(number), "--json", "labels")...)
	if err != nil {
		return nil, nil, err
	}
	var view struct {
	...
	if err := json.Unmarshal([]byte(out), &view); err != nil {
		return nil, nil, fmt.Errorf("parse pull request labels: %w", err)
	}
